...
alone; fitting $E=E_\infty-ah^2+cL^{-p}$ to nine runs separates them. Four
...
from below, the two agreeing to $0.14\%$. Adopting the reference's fixed
boundaries in place of our periodic box shifts the continuum limit by $0.18\%$,
and at finite volume widens the gap rather than closing it. And the identical
...
The mechanism is the one behind fermion doubling in lattice gauge
theory~\cite{nielsen1981}, where an antisymmetric difference operator likewise
acquires spurious zeros of its symbol at the corners of the Brillouin zone, and
behind the checkerboard decoupling familiar from finite-difference fluid
dynamics. What is specific here is the consequence: the null direction is not
merely a spurious oscillation to be filtered but a zero-cost route between
topological sectors, so the invariant that is supposed to protect the soliton
...
Equation~\eqref{eq:h2} covers the quadratic term only. For a general density
$F(\partial\nv)$ the same expansion gives
\begin{equation}
\tfrac12\!\int\!\big[F(D^+\nv)+F(D^-\nv)\big]
 = \int\! F - \frac{h^2}{24}\int\! F''[\partial^2\nv,\partial^2\nv] + O(h^4),
\label{eq:h2gen}
\end{equation}
which reduces to~\eqref{eq:h2} at $F=|\partial\nv|^2$, where $F''=2$. The sign is
negative only where $F$ is convex in $\partial\nv$, and for the quartic density
$F=\tfrac12\sum_{ij}q_{ij}^2$ with
$q_{ij}=\nv\cdot(\partial_i\nv\times\partial_j\nv)$ the second variation
\begin{equation}
F''[v,v]=\sum_{ij}\big[(q'_{ij}\!\cdot\!v)^2
 +2\,q_{ij}\,\nv\cdot(v_i\times v_j)\big]
\end{equation}
is not sign definite. We therefore measure it rather than assert it. Sampling one
analytic configuration at seven spacings---no relaxation intervenes, so every
grid approaches the same continuum value---and fitting $v_0+ah^2$ to the two
terms separately gives $a=-68.8$ for the quadratic term and $a=-214.4$ for the
quartic one. Both lie below the continuum, and the quartic term supplies three
quarters of the total error. Repeating with the fourth-order scheme against $h^4$
returns $+2.3\times10^3$ and $+8.3\times10^3$, above the continuum in both terms.
The $-a\,h^2$ form fitted in Sec.~\ref{sec:charge1} is therefore justified for the
full functional rather than for its quadratic part alone, and the two schemes
bracket the limit term by term.
...
configuration type is reproduced in every sector. These are finite-spacing
values at $h=0.15$ and not continuum limits: the charge-one entry lies $0.9\%$
above Ref.~\cite{sutcliffe2007} here, widening to the $2.2\%$ of
Eq.~\eqref{eq:e1} once the $h\to0$, $L\to\infty$ extrapolation of
Sec.~\ref{sec:charge1} is taken.}
\label{tab:validation}
\begin{ruledtabular}
\begin{tabular}{ccccc}
$\Qh$ & this work & Ref.~\cite{sutcliffe2007} & difference & type (ours / published) \\
\colrule
1 & 1.2149 & 1.204 & $+0.9\%$ & $A_{1,1}$ / $A_{1,1}$ \\
2 & 1.9795 & 1.967 & $+0.6\%$ & $A_{2,1}$ / $A_{2,1}$ \\
3 & 2.7926 & 2.754 & $+1.4\%$ & $A_{3,1}$ / $\widetilde{A}_{3,1}$ \\
4 & 3.5000 & 3.445 & $+1.6\%$ & $A_{2,2}$ / $A_{2,2}$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
two-parameter fit; the boundary condition, by repeating the calculation with
Dirichlet walls; and the whole procedure, by a control with an exact answer.

The remaining methodological difference is the boundary condition itself.
Reference~\cite{sutcliffe2007} truncates the field to the vacuum at the boundary
of a box of side $15.1$; we use a periodic box. For a massless field with
power-law tails the natural expectation is that truncation removes gradient
energy a periodic box retains, with the sign right to explain part of a gap. We
have measured it rather than argued about it.

Repeating the calculation under Dirichlet walls---stencils stopping at the
boundary rather than wrapping, with a collar four cells deep held at the vacuum,
wider than the deepest stencil---gives the paired comparison of
Table~\ref{tab:boundary}. Within a row the seed, the grid and the relaxation
schedule are identical, so the only difference is the boundary. The expectation
is wrong in sign: fixed walls \emph{raise} the energy at every box size, because
clamping the tail forbids it from relaxing, which costs more than truncation
saves. The excess falls monotonically with box size, from $0.69\%$ at $L=10$ to
$0.135\%$ at $L=14$, as two conventions approaching a common limit require.
Fitting~\eqref{eq:extrap} separately to each convention gives
\begin{equation}
E_\infty^{\rm per}=1.2280\pm0.0008, \qquad
E_\infty^{\rm fix}=1.2257\pm0.0012 ,
\label{eq:boundary}
\end{equation}
agreeing to $1.6\sigma$. The boundary condition moves the continuum limit by
$0.18\%$---an order of magnitude too little to account for the discrepancy.
Under the reference's own boundary condition the excess over $1.204$ is
$1.81\%$. The periodic member of this series also reproduces~\eqref{eq:e1}
within its uncertainty, on an independent ladder of grids, which is a
consistency check on both.

\begin{table}[t]
\caption{Paired boundary comparison at $\Qh=1$. Each row is two relaxations from
the same seed on the same grid with the same schedule, differing only in the
boundary condition. Fixed walls lie above periodic everywhere, and the gap
closes as the box grows.}
\label{tab:boundary}
\begin{ruledtabular}
\begin{tabular}{ccccc}
...
Against all of this, Ref.~\cite{sutcliffe2007} itself argues the opposite way,
noting that its grid ``appear[s] to underestimate the energy by an amount of the
order of $1\%$'' and quoting $E\simeq1.21$ from higher-resolution studies. Our
result would put the underestimate nearer $2\%$.

We therefore report $E_1/c_0=1.231\pm0.003$ as our result and the difference from
$1.204$ as open, with the boundary condition no longer among the candidate
explanations. Two differences we cannot probe from outside remain, and we name
them so that they can be checked: the quadrature weights used to sum the energy
density, and the precise discrete form given to the cross product in the quartic
term. Our sum is the rectangle rule, which on a periodic lattice coincides with
the trapezoidal rule and converges faster than any power of $h$ for smooth
periodic integrands, so we do not expect it to carry a percent-level effect; but
it is a difference of convention that only the reference implementation can
settle. What would resolve the discrepancy is an independent implementation
rather than an independent run of ours; the code, including both schemes and both
boundary conventions, is available~\cite{repo}.
...
in this paper were produced on the CPU path, with the single exception of the
paired boundary comparison of Table~\ref{tab:boundary}, which was run on the GPU.
That comparison is between two relaxations sharing a code path, so the
interpolator difference enters both members of a pair alike and cancels in the
