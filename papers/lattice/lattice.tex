...
\begin{abstract}
We recompute the low-charge Faddeev--Skyrme soliton spectrum with both lattice
systematics under explicit control, and find a disagreement with the accepted
charge-one energy that we are unable to remove. Two observations organise the
computation. First, a discretisation whose derivative symbol vanishes at the
Nyquist frequency gives the discrete energy exact flat directions: a checkerboard
mode costs nothing, the Hopf charge unwinds under gradient flow, and the energy
falls below the Vakulenko--Kapitanskii bound. Symmetrising the energy over
one-sided differences removes the null mode while keeping the analytic variation
the exact gradient of the discrete energy. Second, finite spacing and finite
volume bias the energy in \emph{opposite} directions, so neither can be assessed
alone; fitting $E=E_\infty-ah^2+cL^{-p}$ to nine runs separates them. The result
is $E_1/c_0=1.231\pm0.003$ against the published $1.204$, an excess of $2.2\%$.
Three independent checks support it rather than the alternative that our lattice
is at fault: at matched spacing $h=0.100$ we obtain $1.2257$, a $1.8\%$ gap of
which our finite box can account for at most $0.7\%$; a fourth-order accurate
symmetrisation, whose error has the opposite sign, extrapolates from above to
$1.2371$ against the second-order scheme's $1.2354$ from below, agreeing to
$0.14\%$; and the identical procedure applied to the Skyrme model, where the
charge-one energy follows from an ordinary differential equation, recovers that
exact value to $0.03\%$ from runs whose individual errors reach $2\%$. In the
same frame sector we obtain $B=1$--$4$ energies within $0.6$--$0.8\%$ of the
tabulated values and binding energies per baryon within $0.15$ percentage
points. We reproduce the published charge-one to charge-four configuration
types, the sub-linear scaling exponent $0.765$ against $3/4$, and binding
against fission in every sector, so the discrepancy is confined to the absolute
normalisation and not to the structure of the spectrum.
\end{abstract}
